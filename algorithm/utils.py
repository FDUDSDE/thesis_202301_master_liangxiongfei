...
from utils2 import inflate_mask
...
ID_COLOR = {0: '#FFB6C1', 1: '#EE4D4D', 2: '#C67C7B', 3: '#FFD274', 4: '#BEBEBE', 5: '#BFE3E8', 6: '#7BA779',
            ...
            14: '#FFD700', 15: '#727171', 16: '#785A67', 17: '#D3A2C7', 18: '#7FFCBE'}
...
def selectRandomNodes(nd_to_sample, batch_size):
    ...
    for k in range(batch_size):
        ...
        fixed_nodes_state = torch.tensor(np.random.choice(list(range(rooms_num)), size=N,
                                                          replace=False)).cuda()  ##torch.tensor(list(range(rooms_num))).long().cuda() ##
...
def get_mask_cnt(masks: torch.FloatTensor):
    mks = np.zeros(masks.shape)
    # masks[torch.where(masks <= 0)] = 0

    mks[torch.where(masks > 0)] = 1
    # mks = masks.detach().cpu().numpy()
    # mks = np.array(mks, dtype=np.int32)

    res = []
    for mks_i in mks:
        # distance = ndimage.morphology.distance_transform_cdt(mks_i)
        ...
        # labels = segmentation.watershed(-distance, markers, mask=mks_i, connectivity=8)
        labels = measure.label(mks_i, connectivity=2)
        res.append(labels.max())
    res = torch.FloatTensor(res)
    res = res.to(masks.device)
    res = res.reshape(-1, 1)
    return res


def is_neighbor(mask1, mask2):
    intersect_pixel = mask2[torch.where(mask1 > 0)]
    inflate_mask1 = inflate_mask(mask1)
    inflate_intersect_pixel = mask2[torch.where(inflate_mask1 > 0)]
    if len(intersect_pixel) > 0 and len(
            inflate_intersect_pixel) > 0 and intersect_pixel.min() < 0 and inflate_intersect_pixel.max() > 0 and len(
            torch.where(intersect_pixel>0)[0]) <= len(intersect_pixel) / 2:
        return True
    return False


def beautify_room(masks):
    for k in range(len(masks)):
        for i in range(1, 63):
            for j in range(1, 63):
                if masks[k][i][j - 1] > 0 and masks[k][i][j + 1] > 0 and masks[k][i][j] < 0:
                    masks[k][i][j] = -masks[k][i][j]
                if masks[k][i - 1][j] > 0 and masks[k][i + 1][j] > 0 and masks[k][i][j] < 0:
                    masks[k][i][j] = -masks[k][i][j]
        for i in range(1, 63):
            for j in range(1, 63):
                if masks[k][i][j - 1] < 0 and masks[k][i][j + 1] < 0 and masks[k][i][j] > 0:
    ...
    # if mask[i+1][j]*mask[i+1][j+1]>0 and mask[i][j+1]*mask[i+1][j+1]>0 and mask[i][j]*mask[i+1][j+1]<0:
    #     mask[i][j] = -mask[i][j]
    # if mask[i+1][j]*mask[i+1][j-1]>0 and mask[i][j-1]*mask[i+1][j-1]>0 and mask[i][j]*mask[i+1][j-1]<0:
    #     mask[i][j] = -mask[i][j]
    # if mask[i-1][j]*mask[i-1][j-1]>0 and mask[i][j-1]*mask[i-1][j-1]>0 and mask[i][j]*mask[i-1][j-1]<0:
...
def rectangle_room(mask):
    # mask = beautify_room(mask)
    idx = np.where(mask > 0)
    minX, maxX = idx[0].min(), idx[0].max()
    minY, maxY = idx[1].min(), idx[1].max()
    mask[minX:maxX + 1, minY:maxY + 1] = abs(mask[minX:maxX + 1, minY:maxY + 1])
    return mask


def is_overlap(masks, idx, target):
    target_idx = torch.where(target > 0)
    for index in range(len(idx)):
        if masks[idx, 0][index][target_idx].max() > 0:
            return True
    return False
